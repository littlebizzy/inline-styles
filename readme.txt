...
This plugin does 3 main things:

1. converts all CSS to inline <style> for better performance, less conflicts, and less cache/CDN management.
...
Version 1.1.0 update: Now force-removes any hard-coded CSS from page render to avoid conflicts and duplicate CSS. It does this after all HTML is already rendered to avoid conflicts. IE conditional tags are now also stripped:

* For all properly registered external styles, the plugin changes the declaration to inline and loads the file content (absolutizing the background URLs) into specific inline WP variables.

* Any other link tag (with rel="stylesheet" or type="text/css") having a 'wp-content' href URL (introduced usign hooks or template harcoding), and not declared in (1), will be removed.

* Conditional styles are allowed and they will not be converted to inline styles, except for conditionals involving IE, in which case they will be discarded.

Future might have a defined constant to include conditional IE CSS if the community desires this feature.
...
= 1.1.0 =
* strips out all conditional IE styles from inline styles
* removes all hard-coded `wp-content` CSS references (<link...>) from HTML page render
